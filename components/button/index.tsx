  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: 'radial-gradient(circle at 30% 120%, rgba(255,255,255,0.3), transparent 60%)',
        }}
        animate={{
...
export function InfernoButton({ label = 'Overdrive' }) {
  ...
  const handleClick = () => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      spark.className = 'absolute w-2 h-2 bg-orange-400 rounded-full pointer-events-none';
...
export function FrostButton({ label = 'Freeze Time' }) {
  ...
  const handleClick = () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      flake.className = 'absolute w-1.5 h-1.5 bg-white rounded-full pointer-events-none';
      flake.style.left = `${50}%`;
      flake.style.top = `${50}%`;
...
export function GalaxyButton({ label = 'Warp Drive' }) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    ripple.className = 'absolute rounded-full bg-white/20 backdrop-blur-sm pointer-events-none';
    ...
    for (let i = 0; i < 12; i++) {
      const particle = document.createElement('div');
      particle.className = 'absolute rounded-full bg-white pointer-events-none shadow-md';
      particle.style.left = `${x}px`;
      particle.style.top = `${y}px`;
      particle.style.width = '6px';
      particle.style.height = '6px';
  ...
  return (
    <div className="relative inline-block">
      ...
      >
        ...
        <div ref={rippleLayer} className="absolute inset-0 overflow-visible pointer-events-none" />
        <div
          ref={particleLayer}
